...
{
  echo "# ============================================"
  echo "# Owly — Generated by install-owly.sh"
  echo "# Date: $(date -Iseconds)"
  echo "# ============================================"
  echo ""
  echo "# ---- Database ----"
  echo "DATABASE_URL=\"postgresql://postgres:${gen_secret}@db:5432/owly?schema=public\""
  echo ""
  echo "# ---- App ----"
  echo "NEXT_PUBLIC_APP_URL=\"https://$(ask_required 'APP_HOST' 'Bot domain (e.g. bot.mydomain.com)' 'bot.example.com')\""
  echo "NODE_ENV=\"production\""
  echo ""
  echo "# ---- Secrets ----"
  echo "JWT_SECRET=\"$(gen_secret)\""
  echo "WEBHOOK_SECRET=\"$(gen_secret)\""
  echo ""
  echo "# ---- AI Provider (at least one required) ----"
  ask_required 'OPENAI_API_KEY' 'OpenAI API Key (sk-...)' ''
  ask_optional 'OPENAI_API_KEY' 'OpenAI API Key (sk-...)' ''
  ask_optional 'ANTHROPIC_API_KEY' 'Anthropic API Key (sk-ant-...)' ''
  ask_optional 'GROQ_API_KEY' 'Groq API Key (gsk_...)' ''
  ask_optional 'OPENROUTER_API_KEY' 'OpenRouter API Key' ''
  echo ""
  echo "# ---- Channels ----"
  echo "# WhatsApp"
  echo "WHATSAPP_VERIFY_TOKEN=\"$(gen_secret)\""
  echo "WHATSAPP_SHARE_TOKEN=\"$(gen_secret)\""
  echo "WHATSAPP_API_KEY=\"$(gen_secret)\""
  echo ""
  echo "# Telegram (optional)"
  echo "# TELEGRAM_BOT_TOKEN=\"123456:ABC-DEF...\""
  echo "# TELEGRAM_WEBHOOK_URL=\"https://$(echo $APP_HOST)/api/telegram/webhook\""
  echo ""
  echo "# ---- CORS ----"
  echo "CORS_ORIGIN=\"https://$(echo $APP_HOST)\""
  echo ""
  echo "# ---- Snapshots (auto-configured) ----"
  echo "SNAPSHOT_DIR=\"/backups/owly\""
  echo "SNAPSHOT_RETENTION_DAYS=\"7\""
  
} > /tmp/owly-env-inputs.txt

# Actually generate the env file properly
{
  echo "# ============================================"
  echo "# Owly — Generated by install-owly.sh"
  echo "# Date: $(date -Iseconds)"
  ...
  echo ""
  echo "# ---- Database ----"
  echo "DATABASE_URL=\"postgresql://postgres:$(openssl rand -base64 20 | tr -d '/+=' | head -c 24)@db:5432/owly?schema=public\""
  echo ""
  echo "# ---- App ----"
  APP_URL=""
  while [[ -z "$APP_URL" ]]; do
    echo -n "  Bot domain (e.g. bot.mydomain.com): "
    read -r APP_URL
    if [[ -z "$APP_URL" ]]; then log_warn "Domain is required."; fi
  done
  ...
  while [[ -z "$OPENAI_KEY" ]]; do
    ...
    read -rs OPENAI_KEY
    echo ""
    if [[ -z "$OPENAI_KEY" ]]; then log_warn "At least one AI provider is required."; fi
  done
  ...
  echo ""
  echo -n "  Groq API Key (gsk_...) [optional, press Enter to skip]: "
  read -rs GROQ_KEY
  echo ""
  [[ -n "$GROQ_KEY" ]] && echo "GROQ_API_KEY=\"${GROQ_KEY}\""
  
  echo ""
  echo -n "  OpenRouter API Key [optional, press Enter to skip]: "
  ...
  echo "# ---- Channels ----"
  echo "# WhatsApp"
  echo "WHATSAPP_VERIFY_TOKEN=\"$(openssl rand -base64 32)\""
  echo "WHATSAPP_SHARE_TOKEN=\"$(openssl rand -base64 32)\""
  echo "WHATSAPP_API_KEY=\"$(openssl rand -base64 32)\""
  echo ""
  echo "# Telegram (optional — uncomment after creating bot with @BotFather)"
  echo "# TELEGRAM_BOT_TOKEN=\"123456:ABC-DEF...\""
  echo "# TELEGRAM_WEBHOOK_URL=\"https://${APP_URL}/api/telegram/webhook\""
  echo ""
  echo "# ---- CORS ----"
  echo "CORS_ORIGIN=\"https://${APP_URL}\""
  echo ""
  echo "# ---- Snapshots (auto-configured) ----"
  echo "SNAPSHOT_DIR=\"/backups/owly\""
  echo "SNAPSHOT_RETENTION_DAYS=\"7\""
