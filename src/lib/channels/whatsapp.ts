...
export const DISCLAIMER_TEXT = [
  ...
  "",
  "4. *المواكبة النقابية المباشرة*: في الملفات الفردية الدقيقة أو النزاعات الإدارية المعقدة، ندعو الزميلات والزملاء دوماً إلى:",
...
async function startBaileys(retry = 0): Promise<void> {
  ...
  async function handleGroupWelcome(groupJid: string, groupSubject?: string) {
    ...
    try {
      ...
      const welcomeText = `تحية نضالية لجميع الزميلات والزملاء الأعزاء 🇲🇦🌹
    ...
    for (const msg of msgs) {
      ...
      if (!body && isAudio && !msg.key.fromMe) {
        try {
          ...
          await sendText(jid, "عذراً زميلي/زميلتي، تعذر تفريغ الرسالة الصوتية بوضوح. يُرجى التفضل بإعادة إرسال الصوت أو كتابة استفساركم نصياً وسنجيبكم فوراً.");
          continue;
        }
      }
